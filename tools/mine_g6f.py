#!/usr/bin/env python3
"""G6 with the fbseg premise: window S = M[j:kk] inside the region dominated
by pp = M[i] (mid gap M[i+1:j] allowed), u = snd pp.
Claim: g in Gterm u (NT S), hdsub g = maxr1 S ==> ole g (NT (msfx S)).
fbseg u is hereditary into K and T with the SAME u, so this form supports
the GCAT-style induction. Expect 0 if the unification is right.
"""
import sys
sys.path.insert(0, '.')
...
def main():
    ...
    print(f'fbseg windows: {len(seen)}  max-headed visible g: {tot}  G6 violations: {viol}')
    for u, Sp, g in ex:
