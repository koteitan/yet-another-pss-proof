...
If $`a \in \mathrm{ST\_PS}`$, $`\mathrm{tr}\,b = \mathrm{tr}\,a`$ and
$`a \in \mathrm{Acc}_{R_{\mathrm{st}}}`$, then $`b \in \mathrm{Acc}_{R_{\mathrm{st}}}`$.

### Proof

By [T.Acc.intro](Reduction.md#t-Acc.intro) it suffices to show
$`y \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ for every $`y`$ with $`y \mathbin{R_{\mathrm{st}}} b`$.
...
Applying [T.Acc.inv](Reduction.md#t-Acc.inv) to the hypothesis $`a \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ and
$`y \mathbin{R_{\mathrm{st}}} a`$ gives $`y \in \mathrm{Acc}_{R_{\mathrm{st}}}`$. ∎
...
$`c \in \mathrm{ST\_PS}`$ and $`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ holds for every $`n`$ with
$`1 \le n`$, then $`c \in \mathrm{Acc}_{R_{\mathrm{st}}}`$.

### Proof

By [T.Acc.intro](Reduction.md#t-Acc.intro) it suffices to show
$`b \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ for every $`b`$ with $`b \mathbin{R_{\mathrm{st}}} c`$.
...
- $`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$: this is the hypothesis applied to $`n`$.
...
- $`\mathrm{tr}\,b \prec \mathrm{tr}\,(c[n])`$. Then $`b \mathbin{R_{\mathrm{st}}} c[n]`$ holds
  ...
  strict inequality). Applying [T.Acc.inv](Reduction.md#t-Acc.inv) to $`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ gives
...
- $`\mathrm{tr}\,b = \mathrm{tr}\,(c[n])`$. Apply
  ...
  $`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ are all available, and its conclusion is
  $`b \in \mathrm{Acc}_{R_{\mathrm{st}}}`$. ∎
...
Assume (hcof). Then $`M \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ for every $`u \in \mathbb{N}`$ and every
$`M \in W_u`$.

### Proof

Put $`Y := \{\, M \in \mathrm{PairSeq} \mid M \in \mathrm{Acc}_{R_{\mathrm{st}}} \,\}`$; then the claim
...
$`c \in \mathrm{Acc}_{R_{\mathrm{st}}}`$. Distinguish cases according to whether
$`c \in \mathrm{ST\_PS}`$.

**(I) $`c \notin \mathrm{ST\_PS}`$.**
By [T.Acc.intro](Reduction.md#t-Acc.intro) it suffices to show
$`y \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ for every $`y`$ with $`y \mathbin{R_{\mathrm{st}}} c`$.
...
exists, the antecedent is false, and $`c \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ holds.
...
Hence no such $`y`$ exists, the antecedent is false, and $`c \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ holds.

**Branch (2): $`\mathrm{natDom}(c)`$ and $`\forall n \ge 1,\ c[n] \in Y`$.**
By the definition of $`Y`$, the second conjunct says that $`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$
holds for every $`n`$ with $`1 \le n`$. Applying
[T.acc_of_nat_branch](#t-acc_of_nat_branch) to this and to $`c \in \mathrm{ST\_PS}`$ gives
$`c \in \mathrm{Acc}_{R_{\mathrm{st}}}`$.
...
$`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$ for each $`n`$ with $`1 \le n`$.
...
$`c[n] \in \mathrm{Acc}_{R_{\mathrm{st}}}`$.
