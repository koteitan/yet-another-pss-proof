...
`lean/Foo.lean` に、証明の一部ではない補足（設計上の判断、他の経路との関係、
参考文献との対応など）が書かれていることがある。これらは**証明本文より下**に、
**引用ブロック**として書く。証明とひと目で区別できるようにするためである。

```markdown
> `oper` の第 3 分岐をコピー枚数 `n` について展開せず `dropLast` で書くのは、
> `M⟦n⟧` の長さが `n` に依らないことを使うためである。
```

「以下は証明の一部ではない」「`Foo.lean` に記された注記である」のような
**前置きは書かない**。引用ブロックであること自体が証明本文との区別を与えている。
...
代わりに、**ファイル A の中でその記号が最初に現れる 1 箇所だけ**に、
ファイル B の定義へのハイパーリンクを張る。2 回目以降は素の記号で書く。
