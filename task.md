...
- 🚨 **本命＝pure-lex 構文的整礎性（順序数なし, wf.thy, sorry ゼロ・緑）** ［決定 2026-06-10, memory pss-wf-route-purelex-syntactic／lean-wtt-route〕
  ...
  - 🟢 **【Lean 2026-06-16】arg-zone ORDER 路線（live path, PROOF-STATUS.md 参照）**: nrm_order_pres を term-level forest core へ還元。GREEN(sorryAx-free): keystone・mvstep recursion・Rdesc/Rdesc_match・SubBlock 対応(`Gterm_translate_subblock`)・H0clause ST_PS 帰納(`H0clause_diagSeq_le1`/`ST_PS_desc`+`ST_PS_desc_caseB`)・dseg/fbseg。残＝単一 forest core `H0clause_oper_step`(Wttone)＝`not_pfire0_lead1max1_NF`＝`Rdesc_hstep`(Nrmstep)＝「∀K SubBlock of B, olt(translate K)(translate B)」＝§1 firing wall（CollapseResidueMaxo の term-level 双対）。詳細 memory lean-wtt-route / lean PROOF-STATUS.md。
