...
The proof has six chapters.

**Chapter 1.** Pair sequences, their fundamental sequences $`M[n]`$, the standard forms, and the
expansion $`M \Rightarrow N`$ are defined.

**Chapter 2.** A pair sequence $`M`$ is mapped to a term $`\mathrm{tr}(M)`$ of the ternary-tree
notation $`p_a(b)+c`$. Terms are equipped with the subscript-first lexicographic order $`\prec`$.

**Chapter 3.** One expansion step strictly decreases this measure: $`\mathrm{tr}(M[n]) \prec \mathrm{tr}(M)`$.
Hence expansion terminates as soon as $`\prec`$ is well-founded on the image of the standard forms.

**Chapter 4.** The tools that Chapter 5 needs are prepared: the Cantor normal form condition, the
fact that $`\mathrm{tr}`$ is an order isomorphism onto the column-lex order, and the positional
invariants of the columns.

**Chapter 5.** That well-foundedness is proved **without ordinals**, and without any translation into
...
**Chapter 6.** Putting these together gives the well-foundedness of the expansion relation and the
non-existence of infinite expansion sequences.

## Structure of the proof

| | What is proved | Text |
|---|---|---|
| Chapter 1 | pair sequences $`M`$, fundamental sequences $`M[n]`$, standard forms $`\mathrm{ST\_PS}`$, expansion $`M \Rightarrow N`$ | [Pair sequence system](Pss.md) |
| Chapter 2 | the notation $`p_a(b)+c`$, the order $`\prec`$, the translation $`\mathrm{tr}`$ | [Three-branch notation](Term.md) |
| Chapter 3 | decomposition into the branches of expansion, and $`\mathrm{tr}(M[n]) \prec \mathrm{tr}(M)`$ | [Decrease of the measure](Decrease.md) |
| | if $`\prec`$ is well-founded on the image of $`\mathrm{tr}`$, then expansion terminates | [Reduction to termination](Reduction.md) |
| Chapter 4 | the Cantor normal form condition $`\mathrm{cnf}`$, and the copy decomposition $`\mathrm{sh}_d`$ / $`\mathrm{cp}_d`$ | [Cantor normal form condition](Cnf.md) [2](Cnf-2.md) [3](Cnf-3.md) |
...
| Chapter 5 | if $`N \prec M`$, then $`N \preceq M[n]`$ for some $`n`$ | [Bachmann cofinality](Cofinality.md) [2](Cofinality-2.md) [3](Cofinality-3.md) |
...
| Chapter 6 | well-foundedness of the expansion relation, and non-existence of infinite expansion sequences | [Main theorem](Final.md) |
