...
> **既存文献との対応（Buchholz ψ との関係・なぜ `P a b c ≠ ψ_a(b)+c` で問題ないか）**:
> Googology Wiki の議論（P進大好きbot「Please Help me on study of Pair Sequence
> System」2018、リポジトリ親ディレクトリに PDF）で指摘されている通り、PSS の
> (a,b) を「深さ a の ψ_b」と読む素朴な対応は本物の Buchholz OCF では壊れる。
> 原因は正規形条件 β ∈ C_ν(β)：例えば ψ₂(0) > ψ₁(ψ₂(0)) なので
> ψ₀(ψ₁(ψ₂(0))) = ψ₀(ψ₂(0)) と**値が潰れ**、項構文と順序数値が一対一でない。
> 本証明はこの対応を一切仮定しない。すなわち（i）`olt` は全項上では整礎で
> ない（t_k/x_k 降下鎖）ことを把握した上で、整礎性は正規形クラス `wf3`
> （条件 `∀g∈Gterm u b. olt g b` は β∈C_ν(β) の構文版）に制限して自前証明
> （柱2 `wf_olt_wf3`、Buchholz Lemma 2.2 相当、sorry ゼロ）。（ii）上記の潰れ
> の等式そのものを計算する装置が `proj`（臨界部分項への置換）であり、
> `nrm∘translate` は「正規形条件を尊重した正しい Buchholz 読み」を構文的に
> 実装する。（iii）旧 route A（NF の順序数値への直接埋め込み）はまさにこの
> collapse で破棄された（冒頭の凡例参照）。必要な性質は「ψ の値との一致」では
> なく、`olt` の wf3 上整礎性（証明済）と step での `nrm∘translate` 厳密減少
> （`nrm_step_dec`、攻略中）の 2 点のみ。標準形どうしは nrm 後も衝突しない
> （実証 2,643,843 ペアで衝突 0・逆転 0）。
