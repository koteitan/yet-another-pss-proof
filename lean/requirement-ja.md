...
**Mathlib 由来の記号**はこちらに宣言が無いが、節を立てないと定義の置き場所が無くなる。
それを最初に必要とするファイルの先頭に、Mathlib の宣言名を識別子として節を置く
（`Reduction.md` の [D.Acc] と [D.WellFounded]）。これらは §6.6 の 1 対 1 の対象外である。
定理の主張の中に定義を書くと主張の一部に読めてしまうので、そこには置かない。
