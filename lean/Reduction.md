...
[$`\mathrm{Three}`$](Term.md#d-Three) の部分集合 $`\mathrm{NF}`$ を、
[$`M \in \mathrm{ST\_PS}`$](Pss.md#d-ST_PS) なる
[$`M \in \mathrm{PairSeq}`$](Pss.md#d-PairSeq) の
[$`\mathrm{tr}`$](Term.md#d-translate) による像として定める。
