...
H_infix_fail=0; H_prefix_fail=0; olt_fail=0
ex=[]
for M in extra:
    M=list(M)
    ...
    infix_nts=set(); prefix_nts=set()
    n=len(W)
    for j in range(1,n+1): prefix_nts.add(NT(tuple(W[:j])))
    for i in range(n):
    ...
    for g in Glist(0,hb):
        if leadof(g)==k:
            ...
            if g not in infix_nts:
                H_infix_fail+=1
                if len(ex)<8: ex.append(("INFIX",fmtb(hb)[:38],fmtb(g)[:26]))
            if g not in prefix_nts:
                H_prefix_fail+=1
print(f"firing={tot} tied criticals={tied_total}")
print(f"olt g hb (target, must be 0) FAIL={olt_fail}")
print(f"H_infix (tied = NT of infix) FAIL={H_infix_fail}")
print(f"H_prefix (tied = NT of prefix) FAIL={H_prefix_fail}")
for e in ex: print("  ",e)
