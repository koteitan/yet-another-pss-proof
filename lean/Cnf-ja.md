...
$`l\langle i\rangle`$ は次の 2 つの操作の合成として定義されている。ステップ 1 は $`l`$ の第 $`i`$ 要素の探索であり、
$`i \lt \lvert l\rvert`$ のときは「第 $`i`$ 要素 $`l_i`$ が見つかった」という結果を、
$`\lvert l\rvert \le i`$ のときは「見つからなかった」という結果を返す。ステップ 2 はその結果に対し、
見つかったときはその値を、見つからなかったときは $`d`$ を返す。

仮定 $`i \lt \lvert l\rvert`$ よりステップ 1 は「$`l_i`$ が見つかった」を返すから、ステップ 2 の値は $`l_i`$ である。∎
