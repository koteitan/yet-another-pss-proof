...
ntcache={}
def NT(S):
    if S in ntcache: return ntcache[S]
    v=nrm(conv(translate(list(S)))); ntcache[S]=v; return v
...
seen=set(); tot=0; tied_total=0
H_prefix_fail=0; H_infix_fail=0; olt_fail=0; ex=[]
for M in extra:
    M=list(M)
    ...
    tied=[g for g in Glist(0,hb) if leadof(g)==k]
    need_infix=[]
    for g in tied:
        tied_total+=1
        if not lt_term(g,hb): olt_fail+=1
        if g not in prefix_nts:
            H_prefix_fail+=1; need_infix.append(g)
    if need_infix:
        infix_nts=set()
        for i in range(n):
        ...
        for g in need_infix:
            if g not in infix_nts:
                H_infix_fail+=1
                if len(ex)<8: ex.append((fmtb(hb)[:40],fmtb(g)[:28]))
print(f"firing={tot} tied criticals={tied_total}")
print(f"olt g hb (target,must be 0) FAIL={olt_fail}")
print(f"H_prefix FAIL={H_prefix_fail}  (tied not = NT of any prefix)")
print(f"H_infix  FAIL={H_infix_fail}  (tied not = NT of any infix, among prefix-fails)")
for e in ex: print("   infix-fail: hb=",e[0]," g=",e[1])
