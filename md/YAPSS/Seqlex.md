...
<a id="t-blockok_nil"></a>
### 定理 空列は任意の深さのブロック (T.blockok_nil)

**主張** 任意の $`d\in\mathbb{N}`$ に対し $`\mathrm{blockok}(d,[\,])`$。

**証明** [(D.blockok)](#d-blockok) の 3 成分を確かめる。

- 先頭条件：前件 $`[\,]\ne[\,]`$ の証明 $`h`$ からは $`h\,(\mathrm{rfl})`$ により $`\bot`$ が得られるので、任意の結論が従う。
- 下界条件：$`p\in[\,]`$ なる $`p`$ は存在しないので、全称命題は空虚に成り立つ。
- 増分条件：[(T.steps1_nil)](#t-steps1_nil)。∎
