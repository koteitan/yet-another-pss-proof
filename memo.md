...
### ★重要発見 (2026-06-08): width は `<o` 単調（ground は非単調）
Python (`work/ot_order.py`) で検証: principal 340 項で
- **`r <o a ⟹ width r ≤ width a`（違反 0）** ＝ **width 単調**！
- ground は r<o a で r≥a が 17679 件違反（崩壊で ground 上下するので非単調＝予想通り）。

∴ **width で階層化すれば旧 FC 階層と同型の綺麗な PC（前者閉包）が成り立つ**＝
`wdt_mono`（= 旧 `FC_mono_pr` の width 版）が PC を与え、distinguished-set 足場
（Awf/AccBelow/Acc/wf_on_Awf/Acc_imp_acc_pR_step/wf_pR_of_Ifull）は **FC→width 置換でほぼ流用可**。
本丸は within-width（特に width 0＝countable）の closure＝Lemma 3.10/3.12（構造帰納＋正規化）に集約。
- 進捗(続): `wo.thy` に gnd/wdt/norm＋FC_shift/gnd_shift/wdt_shift/FC_norm/wdt_nonneg/shift_wfo,
  `buchholz.thy` に `acc_shift`(acc は shift 不変) すべて緑・コミット済。
- 次: `wdt_mono`(r<o a⟹wdt r≤wdt a, principal) を証明 → width 階層足場 → Ifull_w → 3.10/3.12。
