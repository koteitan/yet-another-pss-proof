...
ペア数列そのもの、その上の親子関係、基本列 `M⟦n⟧`、標準形 `ST_PS`、
そして停止性の対象である展開関係 `step` を定める。
-/
...
/-- The least set of *standard forms*: the pair sequences reachable from a
diagonal `(0,0)(1,1)…(v,v) = diagSeq 0 v` by the expansion `M ↦ M⟦n⟧`
(`n ≥ 1`).  Every standard form begins with `(0,0)`. -/
