...
**例外が 1 つある。** Mathlib 由来の記号はこちらに宣言が無く、節を立てられない（§1.2）。
それを最初に必要とするファイルが、ナビ行の直後で定める。`Reduction.md` が $`\mathrm{Acc}_R`$ と
整礎性についてこれを行う。定理の主張の中に定義を置くと主張の一部に読めてしまうので、そこには置かない。
