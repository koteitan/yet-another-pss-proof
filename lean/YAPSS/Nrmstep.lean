...
/-- **In the `d0 = 0` expansion every grid column sits at level ≥ v0** — the
`b0` analogue of the strict root pivot, giving the valley condition that lets
`le0_retarget`/`le0_shorten` bridge the same-level copy roots. -/
theorem copyExp_b0_ge_v0 {G R : PairSeq} {v0 w0 n : ℕ}
    (hdom : ∀ x ∈ R, v0 < x.1) :
    ∀ y, G.length ≤ y → y < (copyExp G ((v0,w0)::R) 0 n).length →
      v0 ≤ entry (copyExp G ((v0,w0)::R) 0 n) 0 y := by
  intro y hy1 hy2
  set B : PairSeq := (v0,w0)::R with hBdef
  have hL : 0 < B.length := by rw [hBdef]; simp
  have hXlen : (copyExp G B 0 n).length = G.length + n * B.length :=
    copyExp_length ..
  rw [hXlen] at hy2
  obtain ⟨k, q, hk, hq, hyeq⟩ :=
    index_decomp hL (show y - G.length < n * B.length by omega)
...
/-- **Host-edge reconstruction for a `d0 = 0` cross instance**: a row-1 edge
onto a copy-`kt` root (`kt ≥ 1`) sourced in the prefix transfers to the host
row-1 edge `p → G.length`.  `le0_shorten` recovers `le0 X p g` and
`le0_retarget` (valley ≥ v0) carries each prefix ancestor of `g` forward to the
target root for the maximality clause. -/
theorem cross_host_edge_b0 {G R : PairSeq} {v0 w0 n p kt : ℕ} {lp : ℕ × ℕ}
    (hn : 1 ≤ n) (hdom : ∀ x ∈ R, v0 < x.1)
    (hkt : kt < n) (hkt1 : 1 ≤ kt) (hp : p < G.length)
  ...
  have hX0r : entry (copyExp G ((v0,w0)::R) 0 n) 0
    ...
    have h := (entry_copyExp (G := G) (B := (v0,w0)::R) (d0 := 0) (n := n)
      (k := kt) (q := 0) hkt hL).1
    simpa using h
  have hvalley : ∀ y, G.length < y → y < G.length + kt * ((v0,w0)::R).length →
      v0 ≤ entry (copyExp G ((v0,w0)::R) 0 n) 0 y := fun y hy1 hy2 =>
    copyExp_b0_ge_v0 hdom y (by omega) (by omega)
  have hle0Xpt : le0 (copyExp G ((v0,w0)::R) 0 n) p
      (G.length + kt * ((v0,w0)::R).length) := hedge.2.2.2.2.1
  have hle0Xpg : le0 (copyExp G ((v0,w0)::R) 0 n) p G.length := by
    refine le0_shorten hle0Xpt (by omega) hgt (by omega) ?_ ?_
    · rw [hX0g, hX0r]
    · intro y hy1 hy2
      rw [hX0r]
      exact hvalley y hy1 hy2
  ...
  have hgM : G.length < (G ++ ((v0,w0)::R) ++ [lp]).length := by rw [hMlen]; omega
  have hgd : ∀ j, j ≤ G.length →
      (copyExp G ((v0,w0)::R) 0 n).getD j (0,0)
        = (G ++ ((v0,w0)::R) ++ [lp]).getD j (0,0) := by
    intro j hj
    rcases Nat.lt_or_ge j G.length with hjg | hjg
    · rw [copyExp_getD_pre hjg, hostM_getD_pre hjg]
  ...
  refine ⟨by omega, by omega, hp, ?_, ?_, ?_⟩
  · have hep : entry (G ++ ((v0,w0)::R) ++ [lp]) 1 p
        = entry (copyExp G ((v0,w0)::R) 0 n) 1 p := by
      unfold entry; rw [if_neg one_ne_zero, if_neg one_ne_zero,
        (hgd p (by omega)).symm]
    have h4 := hedge.2.2.2.1
    rw [hep, hM1g]
  ...
  · intro j hj
    obtain ⟨hpj, hjg⟩ := hj
    have hjgle : j ≤ G.length := le0_le hjg
    rcases Nat.lt_or_eq_of_le hjgle with hjlt | hjeq
    · have hjX : le0 (copyExp G ((v0,w0)::R) 0 n) j G.length :=
        le0_take_eq htake.symm hgm hgM hgX hjg
      have hjt : le0 (copyExp G ((v0,w0)::R) 0 n) j
          (G.length + kt * ((v0,w0)::R).length) := by
        refine le0_retarget hjX hjlt hgt (by omega) ?_ ?_
        · rw [hX0g, hX0r]
        · intro y hy1 hy2
          rw [hX0r]
          rcases Nat.lt_or_ge G.length y with hgy | hgy
          · exact hvalley y hgy hy2
          · have : y = G.length := by omega
            rw [this, hX0g]
      have hmax := hedge.2.2.2.2.2 j ⟨hpj, hjt⟩
      rw [ht1] at hmax
      have hej : entry (G ++ ((v0,w0)::R) ++ [lp]) 1 j
          = entry (copyExp G ((v0,w0)::R) 0 n) 1 j := by
        unfold entry; rw [if_neg one_ne_zero, if_neg one_ne_zero, hgd j hjgle]
      rw [hM1g, hej]
      exact hmax
    · subst hjeq
...
/-- **The edge dichotomy of the `d0 = 0` expansion, fully assembled and
UNCONDITIONAL** (no tie obligation — the b0 interior tie is refuted, not
routed).  Edges with target in the shared prefix+copy-0 region transfer to the
host (`dichOK_of_take_eq`); later copies use `cross_dichOK_b0` (pre→root),
`dichOK_copy` (within-copy), and the b0 refutations for the rest. -/
theorem copyDichOK_of_b0 {G R : PairSeq} {v0 w0 n : ℕ} {lp : ℕ × ℕ}
    (hn : 1 ≤ n) (hdom : ∀ x ∈ R, v0 < x.1) (hlp2 : lp.2 = 0)
    (hM : dichOK (G ++ ((v0,w0)::R) ++ [lp]))
    (hC9 : lp.2 = 0 → 0 < w0 → ∀ i, 0 < i → i < ((v0,w0)::R).length →
      le0 ((v0,w0)::R) 0 i → (((v0,w0)::R).getD i (0,0)).2 = 0) :
  ...
  have hMlen : (G ++ ((v0,w0)::R) ++ [lp]).length
      = G.length + ((v0,w0)::R).length + 1 := hostM_length ..
  have htX : t < (copyExp G ((v0,w0)::R) 0 n).length := hedge.2.1
  have htake : (copyExp G ((v0,w0)::R) 0 n).take (G.length + ((v0,w0)::R).length)
      = (G ++ ((v0,w0)::R) ++ [lp]).take (G.length + ((v0,w0)::R).length) :=
    copyExp_take_base hn
  rcases Nat.lt_or_ge t (G.length + ((v0,w0)::R).length) with htsmall | htbig
  · exact dichOK_of_take_eq htake hM p q t htsmall htX (by rw [hMlen]; omega)
      hedge hpq hqt hpos
  · obtain ⟨kt, toff, hkt, htoff, htdec⟩ :=
      index_decomp hL (show t - G.length < n * ((v0,w0)::R).length by
        rw [hXlen] at htX; omega)
    ...
    · have hteqr : t = G.length + kt * ((v0,w0)::R).length := by rw [hteq, htoff0]; omega
      rcases Nat.lt_or_ge p G.length with hpg | hpg
      · have hedger : nextrel1 (copyExp G ((v0,w0)::R) 0 n) p
            (G.length + kt * ((v0,w0)::R).length) := by rw [← hteqr]; exact hedge
        have hqtr : q < G.length + kt * ((v0,w0)::R).length := by rw [← hteqr]; exact hqt
        have hres := cross_dichOK_b0 hn hdom hkt hkt1 hpg hlp2 hM hC9 hedger
          q hpq hqtr hpos
        rw [← hteqr] at hres
        exact hres
      · exfalso
        obtain ⟨kp, poff, hkp, hpoff, hpdec⟩ :=
          index_decomp hL (show p - G.length < n * ((v0,w0)::R).length by
            ...
            rw [hXlen] at htX; omega)
        have hpeq : p = G.length + (kp * ((v0,w0)::R).length + poff) := by omega
        have hedger : nextrel1 (copyExp G ((v0,w0)::R) 0 n)
            (G.length + (kp * ((v0,w0)::R).length + poff))
            (G.length + kt * ((v0,w0)::R).length) := by
          rw [← hpeq, ← hteqr]; exact hedge
        have hpltt : kp * ((v0,w0)::R).length + poff < kt * ((v0,w0)::R).length := by
          have : p < t := hedge.2.2.1
          omega
        exact cross_refute_b0_gridroot hdom hkt hpltt hedger
    · rcases Nat.lt_or_ge p (G.length + kt * ((v0,w0)::R).length) with hpr | hpge
      · exfalso
        have hedgei : nextrel1 (copyExp G ((v0,w0)::R) 0 n) p
            (G.length + (kt * ((v0,w0)::R).length + toff)) := by rw [← hteq]; exact hedge
        exact cross_refute_b0_interioredge hdom hlp2 hkt htoffp htoff hpr hC9 hedgei
      · have hqge : G.length + kt * ((v0,w0)::R).length ≤ q := by
          have := le0_le hpq; omega
        set poff := p - (G.length + kt * ((v0,w0)::R).length) with hpoffdef
        set qoff := q - (G.length + kt * ((v0,w0)::R).length) with hqoffdef
        ...
        have hteq2 : t = G.length + kt * ((v0,w0)::R).length + toff := by rw [hteq]; omega
        have htofflt : qoff < toff := by omega
        have hedgeC : nextrel1 (copyExp G ((v0,w0)::R) 0 n)
            (G.length + kt * ((v0,w0)::R).length + poff)
            (G.length + kt * ((v0,w0)::R).length + toff) := by
          rw [← hpeq, ← hteq2]; exact hedge
        have hpqC : le0 (copyExp G ((v0,w0)::R) 0 n)
            (G.length + kt * ((v0,w0)::R).length + poff)
            (G.length + kt * ((v0,w0)::R).length + qoff) := by
          rw [← hpeq, ← hqeq]; exact hpq
        have hposC : 0 < ((copyExp G ((v0,w0)::R) 0 n).getD
            (G.length + kt * ((v0,w0)::R).length + qoff) (0,0)).2 := by
          rw [← hqeq]; exact hpos
        have hres := dichOK_copy hM rfl hkt poff qoff toff htoff hedgeC hpqC
          htofflt hposC
