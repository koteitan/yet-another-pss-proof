[← README](README-ja.md) | [English](Column-3.md) | [Japanese](Column-3-ja.md) | Column [1](Column-ja.md) [2](Column-2-ja.md) **3** [4](Column-4-ja.md)
...
$`L`$ を対の列、$`i \lt \lvert L\rvert`$ とすると $`L\langle i\rangle \in L`$（[D.entry](Pss-ja.md#d-entry)）。
...
$`v_0, w_0 \in \mathbb{N}`$、$`R \in \mathrm{PairSeq}`$（[D.PairSeq](Pss-ja.md#d-PairSeq)）、$`q \in \mathbb{N}`$ とし、
...
$`B := (v_0,w_0) :: R`$、$`E := \mathrm{copyExp}(G,B,0,n)`$（[D.copyExp](Column-2-ja.md#d-copyExp)）とおく。次を仮定する。
...
（$`\mathrm{r1ok}`$ [D.r1ok](Column-2-ja.md#d-r1ok)）
...
$`0 \lt \lvert B\rvert`$ であるから [T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk) を $`q := 0`$ に適用して
$`H\langle \lvert G\rvert\rangle = B\langle 0\rangle = (v_0,w_0)`$ を得る。
[T.hostM_length](Column-2-ja.md#t-hostM_length) より
...
$`p \lt \lvert G\rvert`$ であるから [T.hostM_getD_pre](Column-2-ja.md#t-hostM_getD_pre) より
...
[T.copyExp_getD_pre](Column-2-ja.md#t-copyExp_getD_pre) より $`E\langle p\rangle = G\langle p\rangle`$ である。
...
[T.copyExp_getD_pre](Column-2-ja.md#t-copyExp_getD_pre) より $`E\langle l\rangle = G\langle l\rangle`$ であり、
上の 3 番目の条件と [T.hostM_getD_pre](Column-2-ja.md#t-hostM_getD_pre) より
...
$`0 \lt \lvert B\rvert`$ であるから [T.index_decomp](Column-2-ja.md#t-index_decomp) より
$`k' \lt n`$、$`r \lt \lvert B\rvert`$、$`l - \lvert G\rvert = k'\lvert B\rvert + r`$ をみたす $`k', r`$ が
存在し、$`l = \lvert G\rvert + (k'\lvert B\rvert + r)`$ である。
[T.copyExp_getD_copy](Column-2-ja.md#t-copyExp_getD_copy) より
...
[T.copyExp_getD_copy](Column-2-ja.md#t-copyExp_getD_copy) が使えて
...
[T.index_decomp](Column-2-ja.md#t-index_decomp) より $`k'' \lt n`$、$`rr \lt \lvert B\rvert`$、
...
[T.getD_append_right](Column-2-ja.md#t-getD_append_right) を $`G := G \mathbin{+\!\!+} B`$、$`X := (\ell)`$ に適用して
...
$`\mathrm{r1ok}(M)`$ ならば $`\mathrm{r1ok}(\mathrm{Pred}\,M)`$（[D.Pred](Pss-ja.md#d-Pred)）。
...
- $`\lvert M\rvert \ge 2`$ のとき。$`\mathrm{Pred}\,M = \mathrm{dropLast}\,M`$ であり、
  [T.r1ok_dropLast](Column-2-ja.md#t-r1ok_dropLast) を適用すればよい。∎
...
[T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk)（$`r' \lt \lvert B\rvert`$）より
...
**行 0 の親子関係。** $`\lvert G\rvert + r' \to^M_0 \lvert G\rvert + \lvert B\rvert`$（[D.nextrel0](Pss-ja.md#d-nextrel0)）を示す。
...
[T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk) より $`M_{0,j} = B_{0,rr}`$ であり、
(hafter) より $`v_0 + d_0 \le B_{0,rr}`$、すなわち
$`M_{0,\lvert G\rvert + \lvert B\rvert} \le M_{0,j}`$。

**行 0 の祖先関係。** $`\le^M_0`$（[D.le0](Pss-ja.md#d-le0)）の定義の 3 条件のうち (1), (2) は上の (1), (2) であり、
...
[T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk) より $`M_{1,\lvert G\rvert + r'} = B_{1,r'}`$ である。
...
$`1 \le n`$、$`\mathrm{r1ok}(M)`$、$`\mathrm{steps}_1(M)`$（[D.steps1](Seqlex-ja.md#d-steps1)）ならば
$`\mathrm{r1ok}(M[n])`$（[D.oper](Pss-ja.md#d-oper)）。
...
（[T.oper_bad_blocks](Decrease-ja.md#t-oper_bad_blocks) の 6 つの主張のうち、
...
[T.steps1_iff](Seqlex-ja.md#t-steps1_iff) を
...
[T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk) を適用すると、これは
...
である。
[T.steps1_iff](Seqlex-ja.md#t-steps1_iff) を添字 $`\lvert G\rvert + (\lvert B\rvert - 1)`$ に適用して
...
右辺は [T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk)（$`\lvert B\rvert - 1 \lt \lvert B\rvert`$）より
$`B_{0,\lvert B\rvert - 1} + 1`$ である。

**組み立て。** [T.r1ok_copyExp](Column-2-ja.md#t-r1ok_copyExp) を
...
以上より [T.r1ok_copyExp](Column-2-ja.md#t-r1ok_copyExp) が使えて
...
$`M \in \mathrm{ST\_PS}`$（[D.ST_PS](Pss-ja.md#d-ST_PS)）ならば $`\mathrm{r1ok}(M)`$。
...
[T.diagSeq0_length](Column-2-ja.md#t-diagSeq0_length) より $`\lvert \Delta_0^v\rvert = v + 1`$ であるから
$`j \lt v + 1`$ であり、[T.diagSeq0_getD](Column-2-ja.md#t-diagSeq0_getD) より
...
$`j \lt m`$ に [T.getD_take](Column-2-ja.md#t-getD_take) を適用すると
...
[T.copyExp_length](Column-2-ja.md#t-copyExp_length) より
...
[T.copyExp_getD_pre](Column-2-ja.md#t-copyExp_getD_pre) より $`E\langle j\rangle = G\langle j\rangle`$、
[T.hostM_getD_pre](Column-2-ja.md#t-hostM_getD_pre) より $`H\langle j\rangle = G\langle j\rangle`$ である。
...
[T.index_decomp](Column-2-ja.md#t-index_decomp) を $`L := \lvert B\rvert`$ に適用して
...
$`q \lt \lvert B\rvert`$ に [T.hostM_getD_blk](Column-2-ja.md#t-hostM_getD_blk) を適用すると
