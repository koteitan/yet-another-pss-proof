...
**節は定義か定理のどちらかである。** 記号を導入するものが定義、既に定義された記号だけからなる
命題が真だと主張するものが定理である。定理には証明を付ける。「最小性は次の原理として使う」の
ような、どちらでもない編集上の宣言は書かない。

帰納的に定めた集合は、規則で**閉じている**ことを定義し、閉じているものすべての交わりとして
定義する。すると各規則（構成子）と帰納法の原理は**定理**になり、証明が付く
（D.ST_PS と T.ST_PS.diag / T.ST_PS.oper / T.ST_PS.rec がその形である）。

**Mathlib 由来の記号**と、**`inductive` から自動生成される構成子・再帰子**は
`lean/Foo.lean` に `decl` として現れない。それらの節は §6.6 の 1 対 1 の対象外とし、
識別子には Lean の名前をそのまま使う（`T.ST_PS.rec`, `D.Acc` など）。
...
済ませてはならない。読者がその一歩の正しさを自分で検証できるだけの根拠を伴うこと。
...
1. **帰納法の対象**（自然数か、リストの長さか、項の構造か、`ST_PS` の導出か、
   整礎関係に関する整礎帰納か）
2. **何を示すか**（$`\Phi(t) :\equiv \dots`$ と式で書き、
   「$`\Phi(t) :\equiv \dots`$ を $`t`$ について示す。」の形で結ぶ。
   「帰納法の述語は $`\Phi(t)`$ である。」とは書かない）
3. **基底段**の証明
...
- md に書いた命題は、対応する Lean の宣言と**同じ主張**でなければならない。
  仮定を落とす・強める書き換えをしない。
- 証明の各ステップが Lean 側の証明と対応することを確認する。
